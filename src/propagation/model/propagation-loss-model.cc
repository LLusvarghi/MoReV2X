...
posfile.open("friisPosition.unimo", std::ios_base::app);
posfile << "freq= " << 300000000 / m_lambda << "Hz, " << a->GetPosition () << ", " << b->GetPosition () << "\r\n";
posfile.close();
  ...
  testfile.open("rxPowerFriis.unimo", std::ios_base::app);
  testfile << "distance=" << distance<< "m, loss=" << lossDb <<"dB" << ", TxPower= " << txPowerDbm << "dBm, rxPower= " << txPowerDbm - std::max (lossDb, m_minLoss) << "\r\n";
  testfile.close();
  
  testfile.open("results/sidelink/RxPower_.csv", std::ios_base::app);
    testfile << distance<< "," << lossDb << "," << txPowerDbm << "," << txPowerDbm - std::max (lossDb, m_minLoss) << "\r\n";
  testfile.close();
                                                 ...
                                                 Ptr<MobilityModel> b) const
{
  ...
  if (distance <= dCross)
  ...
  else   // Use Two-Ray Pathloss
    {
      ...
      lossdB = txPowerDbm + rayPr; 

    }
         //trace
     std::ofstream testfile;
     testfile.open("results/sidelink/RxPower_TwoRay_1m.csv", std::ios_base::app);
    testfile << distance<< "," << lossdB << "," << txPowerDbm << "," << txPowerDbm - lossdB<< "\r\n";
  testfile.close();
                                                ...
                                                Ptr<MobilityModel> b) const
{
  ...
  NS_LOG_DEBUG ("distance="<<distance<<"m, reference-attenuation="<< -m_referenceLoss<<"dB, "<<
          ...
          //trace
     std::ofstream testfile;
     testfile.open("results/sidelink/RxPower_LogDistance.csv", std::ios_base::app);
    testfile << distance<< "," << rxc << "," << txPowerDbm << "," << txPowerDbm + rxc << "\r\n";
  testfile.close();
  return txPowerDbm + rxc;
}
...
/*Calculate Deterministic Path Loss*/
/*
   * Two-Ray Ground equation:
   *
   * where Pt, Gt and Gr are in dBm units
 ...
 double m_heightAboveZ = 0.5;
 double m_systemLoss = 1.0;
 double m_frequency = 5.9e9;
 double m_lambda = 299792458.0 / m_frequency;

 double ExpectedRxPowerDbm = 0;



  if (distance <= m_minDistance)
    {
      ExpectedRxPowerDbm = txPowerDbm;
    }
  ...
  if (distance <= dCross)
    {
      ...
      NS_LOG_DEBUG ("Receiver within crossover (" << dCross << "m) for Two_ray path; using Friis");
      NS_LOG_DEBUG ("distance=" << distance << "m, attenuation coefficient=" << pr << "dB");
      ExpectedRxPowerDbm  = txPowerDbm + pr;
  ...
  else   // Use Two-Ray Pathloss
    {
      ...
      ExpectedRxPowerDbm  = txPowerDbm + rayPr; 
...
//Ptr<TwoRayGroundPropagationLossModel> twoRayModel;

//twoRayModel -> SetHeightAboveZ(0.5);
//twoRayModel -> SetFrequency(5.9e9);
...
//double ExpectedRxPowerDbm = twoRayModel -> DoCalcRxPower(txPowerDbm,a,b);
double ExpectedRxPowerW = (std::pow (10.0, ExpectedRxPowerDbm/10.0))/1000.0;
ExpectedRxPowerW+=0; // just use it to suppress compilation warning

double totalRxPowerDbm = ExpectedRxPowerDbm + resultPowerDbm;

          //trace
/*if(distance > 0)
{
  std::ofstream testfile;
  testfile.open("results/sidelink/RxPower_Nakagami_TwoRay_FixedDistance.csv", std::ios_base::app);
  testfile << distance<< "," << txPowerDbm - totalRxPowerDbm << "," << txPowerDbm << "," << totalRxPowerDbm << "\r\n";
  testfile.close();

}*/

  // return resultPowerDbm; ---> Original
  return totalRxPowerDbm;
}
