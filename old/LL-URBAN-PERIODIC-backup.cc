...
*/
  if (((xPosition >= 243) && (xPosition <= 507)) && ((yPosition >= 426) && (yPosition <= 873))){
...
//EventId evento = Simulator::Schedule(Seconds(0.001), UdpClient::Send, this);
  ...
  if (((rxPosX >= 230) && (rxPosX <= 520)) && ((rxPosY >= 426) && (rxPosY <= 873))){
...
BuildingsPropagationLossModel::DoCalcRxPower (double txPowerDbm, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
 ...
 double m_systemLoss = 1.0;
   double sigmaLOS = 3; //dB, for LOS
   double sigmaNLOS = 4; //dB, for NLOS
   double shadowingValue = m_randVariable->GetValue (0.0, (sigmaLOS*sigmaLOS));
  ...
  Vector posB = b->GetPosition ();
   ...
   mLOS = 2.7 * std::exp(-0.01 * (dist - 1)) + 1.0;
  double mNLOS = 0.5;
   double Nj;
   Ptr<GammaRandomVariable> gammaRand = CreateObject<GammaRandomVariable>();
   double fastFading = - 10 * std::log10 (gammaRand -> GetValue (mLOS, 1/mLOS));
...
double dbp = 4 * fc * 1e9 * (hbs1 - 1) * (hbs1 - 1)  / c;
  ...
  bool isVehicleA = 0;
  bool isVehicleB = 0;
  Ptr<LTENodeState> CHECKA = 0;
  Ptr<LTENodeState> CHECKB = 0;
  isVehicleA = nodeStateA -> IsVehicle();
  isVehicleB = nodeStateB -> IsVehicle();
  
 if(isVehicleA && isVehicleB && nodeStateA != CHECKA && nodeStateB != CHECKB){
	if((std::abs(posA.x - posB.x) <= 12) || (std::abs(posA.y - posB.y) <= 12)){
		// OK, LOS values already computed
	}else{
		Nj = std::max(2.8-0.0024*dist,1.84);
		expectedLossWinner = expectedLossWinner + 20 - (12.5 * Nj) + (10 * Nj * std::log10 (dist)) + (3 * std::log10 (fc/5.0));
		shadowingValue = m_randVariable->GetValue (0.0, (sigmaNLOS*sigmaNLOS));
		fastFading = - 10 * std::log10 (gammaRand -> GetValue (mNLOS, 1/mNLOS));
}
...
double PrevX[350], PrevY[350], PrevZ[350], VelX[350], VelY[350], VelZ[350];  //20 is the number of nodes
double MeasInterval;
...
void Print () {
        ...
        //    std::cout << "Measurement Interval "<< MeasInterval << "\n";
   	    ...
   	    if (((pos.x >= 243) && (pos.x <= 507)) && ((pos.y >= 426) && (pos.y <= 873))){
...
main (int argc, char *argv[])
{
  ...
  uint32_t ueCount = 350; // Number of V-UEs 
  bool verbose = true;
  double trep = 1; // [s] the CAM repetition period
...
// Numero di ueResponders deve essere uguale al numero di nodi della simulazione SUMO
  ...
  Ns2MobilityHelper ns2 = Ns2MobilityHelper("/home/merani/Desktop/SumoTraces/Random/Urban/350VEH/RUN1/RUN1.tcl"); 
