...
int TrepPrint = 100; //milliseconds
double PrevX[20], PrevY[20], PrevZ[20], VelX[20], VelY[20], VelZ[20];  //10 is the number of nodes
double MeasInterval;
...
void Print () {
        ...
        //    std::cout << "Measurement Interval "<< MeasInterval << "\n";
            ...
            positFile << Simulator::Now().GetMilliSeconds() << "," << ID << "," << pos.x << "," << pos.y << "," << pos.z << "," << VelX[ID-1] << "," << VelY[ID-1] << "," << VelZ[ID-1] << "\r\n";
...
main (int argc, char *argv[])
{
  ...
  double simTime = 20; // The corrected simulation Time --> this is actually more
  double ueTxPower = 23.0; // [dBm]
  uint32_t ueCount = 40; // Number of V-UEs 
  bool verbose = true;
  double trep = 0.02; // [s] the CAM repetition period
...
//@DOC: Codice per posizionare i nostri nodi; si fa instanziando un MobilityHelper

  uint32_t numLanes = 2;
  uint32_t numUePerLane = ueCount/numLanes;
  
  MobilityHelper mobility;
  for (uint32_t laneIndex = 0; laneIndex < numLanes; laneIndex+=1)
// Perdona gli if stupidi, ma se non faccio così mi da errori strani... probabilmente un bug di conversione numerica nel MobilityHelper
 ...
 //std::cout << "laneOffset= " <<laneOffset<<"\r\n" ;
       mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
                                 "MinX", DoubleValue (distOffset),
                                 "MinY", DoubleValue (4*laneOffset), 
                                 "DeltaX", DoubleValue (dist),
                                 "DeltaY", DoubleValue (4),
                                 "GridWidth", UintegerValue (1500),
                                 "LayoutType", StringValue ("RowFirst"));
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");

    for (uint32_t h = laneIndex * numUePerLane; h < (laneIndex+1)*numUePerLane; h++)
    {
       mobility.Install (ueResponders.Get (h)); 
