...
double T_GenCam[240];
int T_Check_Counter[240], GeneratorCounter[240];
double OriginPos[240], OriginVel[240];
double PosX[240], PosY[240], PosZ[240], VelX[240], VelY[240], VelZ[240];
Ptr<UdpClient> THIS[240]; 
EventId CONTEXT[240];
int N_GenCam = 3;
int CHECK[240], FIRST[240];
...
/*  Salvo in un array la posizione e la velocità del nodo all'istante di generazione del pacchetto, faccio solo per la X perchè siamo in una highway */
...
int TrepPrint = 10; // [ms]
double PrevX[240], PrevY[240], PrevZ[240]; 
double MeasInterval;
int T_Check = 10; // [ms]
...
main (int argc, char *argv[])
{
  ...
  double dist = 20; // [m] : the distance between consecutive V-UEs in a single lane, NON SERVE A NIENTE IN QUESTO CASO
  double simTime = 24; // The corrected simulation Time --> this is actually more
  double ueTxPower = 23.0; // [dBm]
  uint32_t ueCount = 240; // Number of V-UEs 
  bool verbose = true;
  double trep = 0.1; // [s] the CAM repetition period, NON SERVE A NIENTE IN QUESTO CASO
...
*/
  ...
  Ns2MobilityHelper ns2 = Ns2MobilityHelper("/home/merani/Desktop/SumoTraces/Random/3+3Lanes/25s/850VEH/RUN1/RUN1.tcl"); 
  ...
  {  
     ...
     PrevY[ID-1] = pos.y;
     PrevZ[ID-1] = pos.z;
     T_GenCam[ID-1] = 100;  //default value is 1000ms
     T_Check_Counter[ID-1] = 0;
     OriginPos[ID-1] = 0;
     OriginVel[ID-1] = 0;
     VelX[ID-1] = 0;
...
Ipv4AddressHelper ipv4;
 ...
 Ipv4AddressGenerator::Init(Ipv4Address ("225.0.0.0"), Ipv4Mask ("255.0.0.0"));
  ...
  clientApps.Get(0) -> SetStartTime(MilliSeconds (std::round(rand -> GetValue(1, 100))*10));
//  clientApps.Get(0) -> SetStartTime (MilliSeconds (0.0));
  for(uint32_t i = 1; i < ueResponders.GetN(); i++)
  {
      ...
      // clientApps.Get(i)->SetStartTime (MilliSeconds (0.0));
