...
UdpClient::Send (void)
{
  ...
  v2xTag.SetPrsvp ((uint32_t) 50); // selection window of 50ms (tra 5ms e 55ms) 
...
*/
  ...
  std::ofstream filetest;
        filetest.open("/home/merani/Desktop/ns-3.22/results/sidelink/TxFile.txt", std::ios_base::app);
...
/*@DOC: Qui comincia una serie di magheggi sugli indirizzi IP che hanno occupato -da soli- circa 15 giorni di tesi
 ...
 Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable>();
  ...
  if (m_sent < m_count)
    {
      Value = RndExp->GetValue ();
      T_GenCam[nodeId-1] = 50 + Value;
      if (T_GenCam[nodeId-1] < 50){
    	   T_GenCam[nodeId-1] = 50;	
      }
      m_sendEvent = Simulator::Schedule (MilliSeconds (T_GenCam[nodeId-1]), &UdpClient::Send, this); //@DOC: si potrebbe trarre spunto da qui per rischedulare un campionamento periodico della velocità?
    }
}
...
//EventId evento = Simulator::Schedule(Seconds(0.001), UdpClient::Send, this);
  ...
  SeqTsHeader seqTs;
      ...
      if ((rxPosX >= 950) && (rxPosX <= 3050)){
       filetest.open("/home/merani/Desktop/ns-3.22/results/sidelink/RxFile.txt",std::ios_base::app);
...
double PrevX[282], PrevY[282], PrevZ[282], VelX[282], VelY[282], VelZ[282];  //10 is the number of nodes
double MeasInterval;
...
void Print () {
        ...
        positFile.open("/home/merani/Desktop/ns-3.22/results/sidelink/posFile.txt",std::ofstream::app);
        for ( L = i; k != L; ++L)
        {
            Ptr<Node> node = *L;
            ID = node->GetId ();
...
main (int argc, char *argv[])
{
  ...
  double simTime = 49; // The corrected simulation Time --> this is actually more
  double ueTxPower = 23.0; // [dBm]
  uint32_t ueCount = 282; // Number of V-UEs 
  bool verbose = true;
  double trep = 0.05; // [s] the CAM repetition period
...
// For Infrastructure 
  ...
  bool randomV2VSelection = false; // --> Resources are not randomly selected; rather, the Mode4 algorithm is employed
 //uint32_t numPackets = 1;
...
// ad uno scarso SNR risultante da un'attenuazione eccessiva (es. deep fade, lunga distanza tra TX e RX)
 ...
 /* remove("results/sidelink/TxFile.txt");
  ...
  std::ofstream readme;
  readme.open ("/home/merani/Desktop/ns-3.22/results/sidelink/simREADME.txt");
...
*/
  ...
  Ns2MobilityHelper ns2 = Ns2MobilityHelper("/home/merani/Desktop/SumoTraces/Random/3+3Lanes/50s/850VEH/RUN1/RUN1.tcl"); 
