...
namespace ns3 {
   ...
   */
  static const std::bitset<8> ItrpToBitmap[107] =  // values from 108 to 127 are reserved
    {
  ...
  void
  SidelinkCommResourcePool::SetPool (NistLteRrcSap::SlCommResourcePool pool) //NistLteRrcSap::SlCommResourcePool pool is a structure
  {
    //parse information
  ...
  void
  SidelinkCommResourcePool::Initialize () //TODO Create analogous method for V2V with a different no. of pssch and pscch
  {
  ...
  bool
  SidelinkCommResourcePool::IsV2XEnabled ()
  {
    return m_V2XEnabled;
  }
  ...
  SidelinkCommResourcePool::GetV2XSlTransmissions (uint32_t frameNo, uint32_t subframeNo, uint16_t rbStartPscch, uint16_t rbStartPssch, uint16_t nbRbPssch, uint32_t subframeInitialTx, uint8_t SFGap)
{
  //NS_ASSERT_MSG (n < m_nPscchResources, "Requesting resource " << n << " but max is " << m_nPscchResources);
    ...
    std::list<SidelinkCommResourcePool::V2XSidelinkTransmissionInfo> trans;

    
    //36.213 rel 12.5 - 14.2.1.1
    SidelinkCommResourcePool::V2XSidelinkTransmissionInfo first;
...
uint32_t subframe = subframeNo + subframeInitialTx;
    ...
    if (SFGap > 0) //if retransmission is configured
    {
       SidelinkCommResourcePool::V2XSidelinkTransmissionInfo second = trans.front();
       uint32_t secondSubframe = subframe + (uint32_t) SFGap;
       second.subframe.frameNo = (secondSubframe / 10) % 1024;
       second.subframe.subframeNo = secondSubframe % 10;
       first.isThisAReTx = true;
       // the standard does not specify any frequency hopping mechanism for 
       // sidelink Mode 3 and 4, so just keep the same PRB indices of the initial transmission
  ...
  SidelinkCommResourcePool::GetV2XSlTransmissions (uint32_t frameNo, uint32_t subframeNo, uint16_t rbStartPscch, uint16_t rbStartPssch, uint16_t nbRbPssch, uint32_t frameNoNextTx, uint32_t subframeNoNextTx, uint8_t SFGap)
{
  //NS_ASSERT_MSG (n < m_nPscchResources, "Requesting resource " << n << " but max is " << m_nPscchResources);
    ...
    first.subframe.frameNo = frameNoNextTx % 1024; 
    first.subframe.subframeNo = subframeNoNextTx % 10;
    first.rbStartPscch = rbStartPscch;
    first.rbStartPssch = rbStartPssch;
    first.nbRbPssch = nbRbPssch;
    first.isThisAReTx = false;
    ...
    {
       SidelinkCommResourcePool::V2XSidelinkTransmissionInfo second = trans.front();
       uint32_t secondSubframe = subframeNoNextTx + (uint32_t) SFGap;
       second.subframe.frameNo = (frameNoNextTx + secondSubframe / 10) % 1024;
       second.subframe.subframeNo = secondSubframe % 10;
       first.isThisAReTx = true;
       // the standard does not specify any frequency hopping mechanism for 
       // sidelink Mode 3 and 4, so just keep the same PRB indices of the initial transmission
...
*/
  ...
  SidelinkCommResourcePool::GetPsschTransmissions (uint8_t itrp, uint8_t rbStart, uint8_t rbLen)
  {
    ...
    //for now we do not consider hopping, so all the transmissions occur on the same RBs
  ...
  SidelinkCommResourcePool::GetPsschTransmissions (SidelinkCommResourcePool::SubframeInfo periodStart, uint8_t itrp, uint8_t rbStart, uint8_t rbLen)
  {
    ...
    //std::cout << " found " << psschsubframes.size() << " subframes" << std::endl;
    //make sure the number of subframes is a multiple of 4 //TODO Lorenzo: here the allocation for 4 retx takes place
    int tmp = psschsubframes.size () % 4;
    for (int i = 0 ; i < tmp ; i++) {
      psschsubframes.pop_back (); //remove to round the length to a multiple of 4
    }
    //std::cout << " reduced to " << psschsubframes.size() << " subframes" << std::endl;
